...
| SPACEDECK_ENABLED                        | no       | true                                       | use spacedeck instead of excalidraw as whiteboard                                                                                                                                                                                                                                   |
| MAX_GRID_TILES                           | no       | 24                                         | Maximum number of grid tiles a user can select in the conference grid view. Allowed values: 6, 9, 16, 24. Grid sizes larger than this value are hidden from the layout selection.                                                                                                   |
| DEFAULT_GRID_SIZE                        | no       |                                            | Per-meeting default grid size (e.g. 6, 9, 16, 24). When set, the user's grid size selection is stored per meeting in sessionStorage (survives tab reload) and resets to this default when joining another meeting. When unset, the grid size is persisted globally in localStorage. |
