...
describe('useDateFormat - date and time formatting hook', () => {
  ...
  test('format time for invalid date (returns current time in 24h format)', () => {
    const date = new Date('undefined');
    expect(useDateFormat(date, 'time')).toMatch(/(:)/);
  });

  test('format morning time (before 12 pm)', () => {
  ...
  test('format afternoon time (after 12 pm)', () => {
  ...
  test('format date for invalid date (returns current date in en-US format)', () => {
  ...
  test('format date for en-US locale MM/DD/YYYY', () => {
    (useLocale as jest.Mock).mockReturnValue(mockEn);
  ...
  test('format date for de locale DD.MM.YYYY', () => {
